...
[xx, yy, latent{j}] = ...
  irl_rollout(gaussian(mu0, S0), policy, HH, plant, cost);
...
if plotting.verbosity > 1
  lat = cell(1,10);
  for i=1:10
    [~,~,lat{i}] = irl_rollout(gaussian(mu0, S0), policy, HH, plant, cost);
  end
...
save(strcat('scenarios/cartPole_yuan/results/',filename));
